...
val geminiJavadocDir = layout.buildDirectory.dir("emptyJavadoc")

val generateGeminiJavadoc by tasks.registering {
	doLast {
		geminiJavadocDir.get().asFile.mkdirs()
	}
}

val geminiJavadocJar by tasks.registering(Jar::class) {
	dependsOn(generateGeminiJavadoc)
	archiveClassifier.set("javadoc")
	from(geminiJavadocDir)
}
	...
	publishing {
		publications {
			create<MavenPublication>("release") {
				...
				artifact(geminiJavadocJar.get())

				pom {
					name.set("AI Chat Android SDK - Gemini Provider")
					description.set("Gemini provider implementation for the AI Chat Android SDK.")
