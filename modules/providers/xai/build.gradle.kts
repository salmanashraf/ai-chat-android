...
val xaiJavadocDir = layout.buildDirectory.dir("emptyJavadoc")

val generateXaiJavadoc by tasks.registering {
	doLast {
		xaiJavadocDir.get().asFile.mkdirs()
	}
}

val xaiJavadocJar by tasks.registering(Jar::class) {
	dependsOn(generateXaiJavadoc)
	...
	publishing {
		publications {
			create<MavenPublication>("release") {
				...
				artifact(xaiJavadocJar.get())

				pom {
					name.set("AI Chat Android SDK - xAI Provider")
					description.set("xAI/Grok provider implementation for the AI Chat Android SDK.")
